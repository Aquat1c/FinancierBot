module.exports = {
    ...
    execute(client, message, args) {
      if (!message.member.voice.channel) return message.channel.send(`${client.emotes.error} - You're not in a voice channel !`);
        ...
        const filterToUpdate = client.filter.find((x) => x.toLowerCase() === args[0].toLowerCase());
        ...
        filterUpdated[filterToUpdate] = client.distube.getQueue(message).filter[filterToUpdate] ? false : true;

        client.distube.setFilter(message, filterUpdated);

        if (filterUpdated[filterToUpdate]) message.channel.send(`${client.emotes.music} - I'm **adding** the filter to the music, please wait... Note : the longer the music is, the longer this will take.`);
